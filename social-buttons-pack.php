	...
	function sclbttns_admin_init() {
		...
		if ( empty( $bws_plugin_info ) ) {
			$bws_plugin_info = array(
				'id'      => '209',
				'version' => $sclbttns_plugin_info['Version'],
			);
		}
...
if ( ! function_exists( 'sclbttns_settings_page' ) ) {
	...
	function sclbttns_settings_page() {
		...
		<div class="wrap">
			<h1><?php esc_html_e( 'Social Buttons Settings', 'social-buttons-pack' ); ?></h1>
			<noscript><div class="error below-h2"><p><strong><?php esc_html_e( 'Please, enable JavaScript in Your browser.', 'social-buttons-pack' ); ?></strong></p></div></noscript>
...
if ( ! function_exists( 'sclbttns_admin_enqueue_scripts' ) ) {
	...
	function sclbttns_admin_enqueue_scripts() {
		...
		wp_enqueue_style( 'sclbttns_stylesheet', plugins_url( 'css/admin_style.css', __FILE__ ), array(), $sclbttns_options['plugin_option_version'] );

		if ( isset( $_REQUEST['page'] ) && 'social-buttons.php' === $_REQUEST['page'] ) {
			bws_enqueue_settings_scripts();
			bws_plugins_include_codemirror();
			wp_enqueue_script( 'sclbttns_script', plugins_url( 'js/admin-script.js', __FILE__ ), array( 'jquery' ), $sclbttns_options['plugin_option_version'], true );
			wp_enqueue_media();
			wp_localize_script(
				'sclbttns_script',
				'sclbttns_var',
...
if ( ! function_exists( 'sclbttns_wp_enqueue_scripts' ) ) {
	...
	function sclbttns_wp_enqueue_scripts() {
		...
		wp_enqueue_style( 'sclbttns_stylesheet', plugins_url( 'css/style.css', __FILE__ ), array(), $sclbttns_options['plugin_option_version'] );
	}
}
